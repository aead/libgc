...
impl<'a> Converter<'a> {
    ...
    fn open_file(&self, filename: &'static str) -> IOResult<LineWriter<File>> {
        ...
        if fpath.exists() && !fpath.is_file() {
            return Err(Error::new(ErrorKind::AlreadyExists,
                                  format!("{} is a folder", fpath.display())));
        }
        ...
        if !path.is_dir() {
            if !path.exists() {
                ...
                return Err(Error::new(ErrorKind::NotFound,
                                      format!("{} isn't a folder", path.display())));
            }
        }
        Ok(Converter {
            path: path,
    ...
    pub fn replace_not_gates(&self,
                             gates: &Vec<Gate>,
                             cons: Option<Constant>)
                             -> (Vec<Gate>, Option<Constant>) {
